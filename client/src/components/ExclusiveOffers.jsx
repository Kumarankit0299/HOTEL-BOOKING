...
const ExclusiveOffers = () => {
  return (
    <div className='flex flex-col items-center -x-6 md:ox-16 lg:px-24 xl:px-32 pt-20 pb-30'>
        <div className='flex flex-col md:flex-row items-center justify-between w-full'>
            <Title align='left' title='Exclusive offers' subTitle='Take advantage of our limited-time offerers and special package to enhance your stay and create unforgettable memories.' />
        ...
        <div className='grid grid-cols-1 lg:grid-cols-3 gap-6 mt-12'>
             {exclusiveOffers.map((item)=>(
                    <div key={item._id} className='group relative flex flex-col items-start justify-between gap-1 pt-12 md:pt-18 px-4 rounded-xl text-white bg-no-repeat bg-cover bg-center' style={{backgroundImage:`url(${item.image})`}}>
                        <p className='px-3 py-1 absolute top-4 left-4 text-black bg-white text-grey-800 font-medium rounded-full'>{item.priceOff}% OFF</p>
                        <div>
